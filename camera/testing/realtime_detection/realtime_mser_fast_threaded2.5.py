#!/usr/bin/env python3
"""
Real-time MSER Digit Detection with Balanced Preprocessing
A middle-ground approach between simple and enhanced preprocessing
"""
...
class BalancedMSERDigitDetector:
    def __init__(self):
        ...
        # Balanced preprocessing parameters
        self.clahe_clip_limit = 1.5  # Less aggressive than Threaded3
        self.clahe_tile_size = (8, 8)
        self.contrast_alpha = 1.1    # Less aggressive than Threaded3
        self.brightness_beta = 5     # Less aggressive than Threaded3
        
    def enhance_contrast_clahe(self, gray):
        """Apply CLAHE (Contrast Limited Adaptive Histogram Equalization) - balanced version"""
    ...
    def enhance_contrast_linear(self, gray):
        """Apply linear contrast enhancement - balanced version"""
    ...
    def apply_bilateral_filter(self, gray):
        ...
        return cv2.bilateralFilter(gray, 5, 50, 50)  # Smaller kernel than Threaded3
    
    def preprocess_frame_balanced(self, frame):
        """Balanced preprocessing - between simple and enhanced"""
        # Convert to grayscale
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        
        # Step 1: Light noise reduction with bilateral filter
        denoised = self.apply_bilateral_filter(gray)
        
        # Step 2: CLAHE for local contrast enhancement (less aggressive)
        clahe_enhanced = self.enhance_contrast_clahe(denoised)
        
        # Step 3: Light linear contrast enhancement
        ...
        # Step 6: Light morphological operations to clean up small noise
    ...
    def detect_balanced(self, frame):
        """Detect objects using balanced preprocessing"""
        # Preprocess frame with balanced approach
        processed = self.preprocess_frame_balanced(frame)
        ...
        for region in regions:
            ...
            confidence = weight * 0.65 + 0.35  # Between Threaded2 and Threaded3
...
class BalancedMSERThreadedDetector:
    def __init__(self, stream_url="http://192.168.1.100:8080/video"):
        self.stream_url = stream_url
        self.detector = BalancedMSERDigitDetector()
        ...
        self.detection_timeout = 0.75  # Between Threaded2 (0.5) and Threaded3 (1.0)
        self.detection_history = deque(maxlen=3)  # Smaller history than Threaded3
    ...
    def detection_worker(self):
        ...
        while self.running:
            try:
                ...
                # Perform detection
                detections = self.detector.detect_balanced(frame)
    ...
    def get_latest_detections(self):
        """Get latest detection results with balanced caching"""
    ...
    def draw_detection_overlay(self, frame, detections):
        ...
        for i, det in enumerate(detections):
            ...
            # Only draw if confidence is above 0.6
            if confidence < 0.6:
                continue
    ...
    def run(self):
        ...
        print("Balanced MSER Digit Detection Started")
        ...
        try:
            while True:
                ...
                # Display frame
                cv2.imshow("Balanced MSER Digit Detection", display_frame)
                ...
                if key == ord('q'):
                    ...
                    filename = f"Balanced_MSER_Detection_{timestamp}.png"
...
if __name__ == "__main__":
    detector = BalancedMSERThreadedDetector()
    detector.run()
